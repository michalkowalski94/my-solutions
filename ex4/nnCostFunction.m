...
for i = 1:num_labels,
  yi = y == i;
  Hi = H(:,i);
  Ji = (-1/m)*sum(yi.*log(Hi)+(1-yi).*log(1-Hi));
  J = J + Ji;
end;
%Regularization
r_Theta1 = Theta1(:,2:end);
r_Theta2 = Theta2(:,2:end);
regular = (lambda/(2*m))*(sum(sum(r_Theta1.^2))+sum(sum(r_Theta2.^2)));
J = J.+regular;
...
Theta1_grad(:,2:end) = (Theta1_grad(:,2:end)+((lambda/m)*r_Theta1));
Theta2_grad(:,2:end) = (Theta2_grad(:,2:end)+((lambda/m)*r_Theta2));  
